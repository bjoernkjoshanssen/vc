...
maxSize = 0
for dimensionToCheck in range(0, 12):
	for e in itertools.product((0,1),repeat=8):
		if len([i for i in range(0,8) if e[i] == 1]) == dimensionToCheck:
			toShatter = set([theWords[i] for i in range(0,8) if e[i] == 1])
	...
	print "the greatest number of subsets of a set of " + str(dimensionToCheck) + " words of length " + str(wordLength) + " that can be accepted by 2-state automata is " + str(maxSize)
	print